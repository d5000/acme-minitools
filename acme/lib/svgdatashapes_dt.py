...
def dateformat(format=None):
    ...
    global p_dtformat
    if format is None:
        raise AppDt_Error("dateformat() expecting 'format' arg")
    p_dtformat = format
    return True
...
def toint(dateval=None):
    ...
    try:
        ...
        raise AppDt_Error("toint() got bad datetime value: " + str(dateval) + " (expecting format of " + p_dtformat + ")")
    return utime 
...
def make(utime, fmt=None):
    ...
    try:
        ...
        raise AppDt_Error("nicedt error on utime: " + str(utime) + " and format: " + p_dtformat)
    return outstr
...
def datediff(val1, val2, result="days"):
    ...
    try:
        dt1 = d.datetime.strptime(val1, p_dtformat)
    except:
        raise AppDt_Error("datediff() invalid val1 arg: " + str(val1))
    try:
        dt2 = d.datetime.strptime(val2, p_dtformat)
    except:
        raise AppDt_Error("datediff() invalid val2 arg: " + str(val2))
    if result != "seconds":
...
def daterange(column=None, datarows=None, nearest=None, inc=None, stubformat=None, 
    ...
    if nearest is None:
        raise AppDt_Error("findrange() requires a nearest= arg ")
    if inc is None:
        inc = nearest
    # if inc != nearest:
    ...
    if stubformat is None:
        stubformat = p_dtformat

    # find raw min and max
    dmin = 999999999999999999999999999
    dmax = -999999999999999999999999999 
    for row in datarows:
        if dfindex == -1:
    ...
    if nearest[-6:] != "minute":
        dtmin.replace(minute=0)
        dtmax.replace(minute=0)   # usually zero out minutes
    ...
    if nearest == "year":
        ...
        newmon = int(((dtmin.month / 4) * 3) + 1)
        dtmin = dtmin.replace(month=newmon, day=1, hour=0)
        newmon = int((((dtmax.month / 4) + 1) * 3) + 1)
        yr = dtmax.year
        if newmon >= 12:
            newmon = 1
            r += 1
        ...
        if nearest[:8] == "week_day":
            dmin -= ((7 - int(nearest[-1:])) * 86400)
        ...
        raise AppDt_Error("findrange got unrecognized nearest= arg: " + str(nearest))
    ...
    if inc2 is not None and stub2first is True:
        stub2 = dtcur.strftime(stub2format)
        if stub2place == "prepend":
            stub = stub2 + stub 
        elif stub2place == "replace":
            stub = stub2 
        else:
            stub = stub + stub2
    ...
    while iloop < 500:   # sanity backstop
        ...
        if inc == "month":
        ...
        elif inc == "week" or inc[:8] == "week_day":
            ...
            raise AppDt_Error("findrange() does not recognize inc=" + str(inc))

        if inc not in ["month", "3month"]:
