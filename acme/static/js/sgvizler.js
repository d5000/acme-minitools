...
(function (window, undefined) { // trick: safekeep 'undefined', and minify.
    ...
    S.visualization = (function () {
        ...
        /**
         * Make an Address panel
         * 
         * @class sgvizler.visualization.Panel
         * @extends sgvizler.charts.Chart
         * @constructor
         * @param {Object} container The container element where the
         * chart will be drawn.
         ...
         * @since 0.3.0
         **/
        C.AddressPanel = charts.add(modSC, "AddressPanel",
            function (data, chartOpt) {
                var r, noRows = data.getNumberOfRows(),
                    opt = chartOpt,
                    clip = 'http://purl.org/net/bel-epa/ccy#C',
                    panel = '<div class="ui cards">',
                    url, link, net = 'test';

                for (r = 0; r < noRows; r += 1) {
                    var tx = data.getValue(r, 0),
                        date = new Date(data.getValue(r, 1)*1000),
                        amount = data.getValue(r, 2),
                        stype = data.getValue(r, 3);
                    url = tx.substring(clip.length, tx.length-4);
                    link = url.substring(0, 14) + '....' + url.substring(url.length - 8, url.length);
                    panel += '<div class="ui card">';
                    panel += ' <div class="content">';
                    panel += '  <div class="ui small header">';
                    panel += `   <span class="date">${date}</span>`;
                    panel += `   <span class="val"><a href="/${net}/tx/${url}">${link}</a></span>`;
                    panel += '  </div>';
                    panel += `  <div class="meta">${amount}</div>`;
                    panel += '  <div class="description">';
                    panel += `   <span class="attr"><i class="yellow wizard icon"></i>${stype}</span>`;
                    panel += '  </div>';
                    panel += ' </div>';
                    panel += '</div>';
                }
         ...
         * @since 0.3.0
         **/
        C.AddressList = charts.add(modSC, "AddressList",
            function (data, chartOpt) {
                var r, noRows = data.getNumberOfRows(),
                    opt = chartOpt,
                    clip = 'http://purl.org/net/bel-epa/ccy#C',
                    panel = '<div class="ui items">',
                    url, link, icon, net;

                // console.log("qurl = " + endpointURL);
                for (r = 0; r < noRows; r += 1) {
                    var tx = data.getValue(r, 0),
                        date = new Date(data.getValue(r, 1)*1000),
                        ...
                        stype = data.getValue(r, 3);
                    url = tx.substring(clip.length, tx.length-4);
                    if (S.defaults.query.endpoint.endsWith('slmtchain')) {
                        net = 'test'
                    } else {
                        net = 'main'
                    }
                    link = url.substring(0, 32) + '....' + url.substring(url.length - 8, url.length);
                    if (stype == 'pubkey') {
                        icon = 'key';
                    } else {
                        icon = 'script';
                    }
                    panel += `<div class="ui small item">
                     <i class="yellow ${icon} middle aligned icon"></i>
                     <div class="content">
                      <span class="ui small header"><span class="date">${date}</span></span> <span class="sotto">(${humanedate})</span>
                       <span class="val"><a href="/${net}/tx/${url}"><span class"stet">${link}</a></a></span>
                     </div>
                     <div class="meta">${amount}</div>
                    </div>`

                    // panel += '<div class="ui item">';
                    // panel += ' <div class="content">';
                    // panel += '  <div class="ui small header">';
                    // panel += `   <span class="date">${date}</span>`;
                    // panel += `   <span class="val"><a href="/${net}/tx/${url}">${link}</a></span>`;
        ...
        **/
        ...
        C.Table = charts.add(modSC, "Table",
            function (data, chartOpt) {
                ...
                table = util.createHTMLElement('table', {"class": "ui inverted table"}, rows);
                ...
                linkify = function (url, arraySyntax) {
                    ...
                    if (link) {
                        ...
                        if (arraySyntax) {
                            ...
                            if (href.startsWith('http://purl.org/net/bel-epa/ccy#C'))
                                href = '/tx/' + href.substring(
                                    'http://purl.org/net/bel-epa/ccy#C'.length, href.length)
                            result = '<a href=' + href + '>' + link.substring(5, link.length) + '</a>';
                        }
                    } else { // If it is not a link, then just pass it through.
                        if (S.util.isNumber(url)) {
                            if (url > 1490000000) {
                                try {
                                    url = new Date(url * 1000);
                                } catch(err) {
                                    url = url
                                }
                            } else {
                                url = '<span class="val">' + url + '</span>'
                            }
                        }
                        result = url;
                    }
